...
public class FightPresentationManager : MonoBehaviour
{
    ...
    private const string DigitalRimGlowName = "DigitalRimGlow";
    ...
    private GameObject scanDogAVisualArt;
    private GameObject scanDogBVisualArt;
    ...
    private GameObject fighterADogIdentityArt;
    private GameObject fighterBDogIdentityArt;
    ...
    private GameObject traitAuraA;
    private GameObject traitAuraB;
    private GameObject traitSlashA;
    private GameObject traitSlashB;
    ...
    public void PresentFightResult(Dog dogA, Dog dogB, int dogAHealth, int dogBHealth)
    {
        ...
        if (dogAHealth > dogBHealth)
        {
            ...
            ApplyDogIdentityPortraitResultVisual(fighterADogIdentityArt, true, false, Color.cyan);
        ...
        if (dogBHealth > dogAHealth)
        {
            ...
            ApplyDogIdentityPortraitResultVisual(fighterADogIdentityArt, false, false, Color.cyan);
        ...
        ApplyDogIdentityPortraitResultVisual(fighterADogIdentityArt, false, true, Color.cyan);
    ...
    void AssignExistingArenaTransforms()
    {
        ...
        Transform dogIdentityArtA = arenaRoot.transform.Find("FighterA_DogIdentityArt");
        Transform dogIdentityArtB = arenaRoot.transform.Find("FighterB_DogIdentityArt");
        ...
        if (dogIdentityArtA != null)
        {
            fighterADogIdentityArt = dogIdentityArtA.gameObject;
        }

        if (dogIdentityArtB != null)
        {
            fighterBDogIdentityArt = dogIdentityArtB.gameObject;
        }
    ...
    void CreateDogIdentityArtIfNeeded()
    {
        if (arenaRoot == null)
        {
            return;
        ...
        if (fighterADogIdentityArt == null)
        {
            fighterADogIdentityArt = CreateSingleDogIdentityArt("FighterA_DogIdentityArt");
        }

        if (fighterBDogIdentityArt == null)
        {
            fighterBDogIdentityArt = CreateSingleDogIdentityArt("FighterB_DogIdentityArt");
        }

        UpdateDogIdentityArtPositions();
    }

    GameObject CreateSingleDogIdentityArt(string objectName)
    {
        ...
        GetDogIdentitySpriteRenderer(artObject);
    ...
    void UpdateDogIdentityArtPositions()
    {
        UpdateSingleDogIdentityArtPosition(fighterADogIdentityArt, fighterATransform, currentDogImprintA, true);
        UpdateSingleDogIdentityArtPosition(fighterBDogIdentityArt, fighterBTransform, currentDogImprintB, false);
    }

    void UpdateSingleDogIdentityArtPosition(GameObject artObject, Transform fighterTransform, Dog dog, bool isFighterA)
    {
    ...
    void SetDogImprintArtVisible(GameObject artObject, bool visible)
    {
        if (artObject != null)
        {
    ...
    void SetDogIdentityArtVisible(GameObject artObject, bool visible)
    {
        if (artObject != null)
        {
            artObject.SetActive(visible);
    ...
    bool ApplyBreedArchetypeArtToFighter(GameObject artObject, Dog dog, bool isFighterA, int currentHealth, int maxLikelyHealth)
    {
        ...
        foreach (string resourceName in resourceNames)
        {
            if (TryLoadBreedArchetypeSpriteOrTexture(resourceName, out Sprite sprite, out Texture2D texture))
            {
                if (sprite != null)
                {
                    ConfigureBreedArchetypeSpriteArt(artObject, sprite, dog, isFighterA, currentHealth, maxLikelyHealth, resourceName, DogVisualMode.ArenaFighter);
                }
                else
                {
                    ConfigureBreedArchetypeTextureArt(artObject, texture, dog, isFighterA, currentHealth, maxLikelyHealth, resourceName, DogVisualMode.ArenaFighter);
                }
    ...
    List<string> GetAvailableBreedArtResourceNames(Dog dog)
    {
        ...
        {
            AddBreedArtVariantResourceNames(resourceNames, "dog_imprint_shepherd_hybrid_variant", dog);
            AddBreedArtVariantResourceNames(resourceNames, "dog_imprint_bully_hybrid_variant", dog);
            AddBreedArtVariantResourceNames(resourceNames, "dog_imprint_hybrid_variant", dog);
            return resourceNames;
        }

        if (isHybridBreed)
        {
            AddBreedArtVariantResourceNames(resourceNames, GetHybridSpecificResourceBaseName(breedText), dog);
    ...
    void AddBreedArtVariantNames(List<string> resourceNames, BreedVisualArchetype archetype, Dog dog)
    {
        ...
        AddBreedArtVariantResourceNames(resourceNames, baseName, dog);
    }

    void AddBreedArtVariantResourceNames(List<string> resourceNames, string baseResourceName, Dog dog)
    {
        List<string> availableResources = GetAvailableBreedArtVariantResourceNames(baseResourceName);
        ...
        string selectedResource = PickDeterministicBreedArtVariant(availableResources, dog, baseResourceName);
        AddBreedArtResourceName(resourceNames, selectedResource);

        for (int i = 0; i < availableResources.Count; i++)
        {
            AddBreedArtResourceName(resourceNames, availableResources[i]);
        }
    }

    List<string> GetAvailableBreedArtVariantResourceNames(string baseResourceName)
    {
        ...
        for (int variantNumber = 1; variantNumber <= MaxBreedArchetypeVariantNumber; variantNumber++)
        {
    ...
    string PickDeterministicBreedArtVariant(List<string> availableResources, Dog dog, string baseResourceName)
    {
        ...
        string key = $"{GetDogVariantIdentityKey(dog)}|{GetDogBreedText(dog)}|{baseResourceName}";
        int selectedIndex = Mathf.Clamp(
            Mathf.FloorToInt(GetStableNameHash01(key) * availableResources.Count),
            0,
            availableResources.Count - 1);
    ...
    string GetHybridSpecificResourceBaseName(string breedText)
    {
        ...
        if (IsShepherdBullyHybridText(breedText))
        {
            return "dog_imprint_shepherd_hybrid_variant";
        }

        if (ContainsIronRottBreedText(breedText) && ContainsGuardMastiffBreedText(breedText))
        {
            return "dog_imprint_rott_mastiff_hybrid";
        }

        if (ContainsBullyBreedText(breedText))
        {
            return "dog_imprint_bully_hybrid_variant";
        }
    ...
    bool ContainsGuardMastiffBreedText(string breedText)
    {
        if (string.IsNullOrWhiteSpace(breedText))
        {
            return false;
    ...
    bool ContainsIronRottBreedText(string breedText)
    {
        if (string.IsNullOrWhiteSpace(breedText))
        {
            return false;
    ...
    string GetDogVariantIdentityKey(Dog dog)
    {
        if (dog == null)
        {
            return "dog";
        ...
        {
            return dog.dogId.Trim();
        }

        return GetDogDisplayName(dog, "dog");
    }
    ...
    void ConfigureBreedArchetypeSpriteArt(
        GameObject artObject,
        Sprite sprite,
        Dog dog,
        bool isFighterA,
        ...
        int maxLikelyHealth,
        string resourceName,
        DogVisualMode visualMode = DogVisualMode.ArenaFighter
    )
    {
        ...
        HideDogIdentitySprite(artObject);
        ...
        spriteRenderer.color = GetBreedArchetypeArtTint(dog, isFighterA, currentHealth, maxLikelyHealth, visualMode);
        ...
        ApplyDogVisualModeEffects(artObject, isFighterA, visualMode);
    }

    void ConfigureBreedArchetypeTextureArt(
        GameObject artObject,
        Texture2D texture,
        Dog dog,
        bool isFighterA,
    ...
    )
    {
        ...
        Color tintColor = GetBreedArchetypeArtTint(dog, isFighterA, currentHealth, maxLikelyHealth, visualMode);
    ...
    bool ApplyDogPortraitIdentityArt(GameObject artObject, Dog dog, bool isFighterA, int currentHealth, int maxLikelyHealth, DogVisualMode visualMode)
    {
        if (artObject == null || dog == null)
        {
            SetDogIdentityArtVisible(artObject, false);
            return false;
        }

        Sprite dogSprite = ResolveDogPortraitSprite(dog);
    ...
    void ConfigureDogIdentitySpriteArt(GameObject artObject, Sprite sprite, Dog dog, bool isFighterA, int currentHealth, int maxLikelyHealth, DogVisualMode visualMode)
    {
        ...
        HideBreedArchetypeVisualChildren(artObject);
    ...
    Color GetDogIdentitySpriteTint(Dog dog, bool isFighterA, int currentHealth, int maxLikelyHealth, DogVisualMode visualMode)
    {
        ...
        Color identityColor = ApplyHealthCorruptionVisual(GetDogIdentityColor(dog, isFighterA), healthPercent);
        Color hologramBase = isFighterA
            ? new Color(0.35f, 1f, 1f, 0.84f)
    ...
    Vector3 GetDogIdentitySpriteScale(Sprite sprite)
    {
        ...
        float scale = DogIdentitySpriteTargetHeight / sprite.bounds.size.y;
        float scaledWidth = sprite.bounds.size.x * scale;

        if (scaledWidth > DogIdentitySpriteMaxWidth && scaledWidth > 0f)
        {
            scale *= DogIdentitySpriteMaxWidth / scaledWidth;
        }

        return new Vector3(scale, scale, scale);
    }

    SpriteRenderer GetDogIdentitySpriteRenderer(GameObject artObject)
    {
        if (artObject == null)
        {
            return null;
        ...
        Transform spriteTransform = artObject.transform.Find("DogIdentitySprite");
        ...
        {
            spriteObject = new GameObject("DogIdentitySprite");
    ...
    SpriteRenderer GetExistingDogIdentitySpriteRenderer(GameObject artObject)
    {
        if (artObject == null)
        {
            return null;
        ...
        Transform spriteTransform = artObject.transform.Find("DogIdentitySprite");
        return spriteTransform != null ? spriteTransform.GetComponent<SpriteRenderer>() : null;
    }

    void HideDogIdentitySprite(GameObject artObject)
    {
        SpriteRenderer spriteRenderer = GetExistingDogIdentitySpriteRenderer(artObject);
    ...
    void HideBreedArchetypeVisualChildren(GameObject artObject)
    {
        SpriteRenderer breedSprite = GetExistingBreedArchetypeSpriteRenderer(artObject);

        if (breedSprite != null)
        {
            breedSprite.enabled = false;
        }
    ...
    Color GetBreedArchetypeArtTint(Dog dog, bool isFighterA, int currentHealth, int maxLikelyHealth, DogVisualMode visualMode = DogVisualMode.ArenaFighter)
    {
        if (visualMode == DogVisualMode.SafeReal)
        {
        ...
        Color digitalBase = isFighterA
            ? new Color(0.24f, 0.95f, 1f, 0.82f)
            : new Color(0.36f, 0.72f, 1f, 0.82f);
        float identityWeight = visualMode == DogVisualMode.ArenaFighter ? 0.4f : 0.26f;
        Color tintColor = Color.Lerp(digitalBase, finalColor, identityWeight);
        tintColor.a = visualMode == DogVisualMode.ArenaFighter
            ? Mathf.Lerp(0.62f, 0.84f, Mathf.Clamp01(healthPercent))
            : 0.78f;
        return tintColor;
    }

    void ApplyDogVisualModeEffects(GameObject artObject, bool isFighterA, DogVisualMode visualMode)
    {
        if (artObject == null)
        {
            return;
        ...
        bool shouldShowDigitalEffects = visualMode != DogVisualMode.SafeReal;
        SetDogDigitalEffectChildrenVisible(artObject, shouldShowDigitalEffects);

        if (!shouldShowDigitalEffects)
        {
            return;
        }

        Color accentColor = isFighterA
            ? new Color(0.08f, 0.95f, 1f, 1f)
            : new Color(0.28f, 0.68f, 1f, 1f);
        float effectScale = visualMode == DogVisualMode.ArenaFighter ? 1.08f : 0.96f;

        SetNamedChildVisible(artObject.transform, DigitalRimGlowName, false);

        CreateOrUpdateDigitalEffectBar(
            artObject,
            $"{DigitalRimGlowPrefix}Top",
            new Vector3(0f, 1.5f, -0.065f),
            new Vector3(1.52f * effectScale, 0.025f, 0.025f),
            accentColor
        );
        CreateOrUpdateDigitalEffectBar(
            artObject,
            $"{DigitalRimGlowPrefix}Bottom",
            new Vector3(0f, 0.18f, -0.065f),
            new Vector3(1.52f * effectScale, 0.025f, 0.025f),
            accentColor
        );
        CreateOrUpdateDigitalEffectBar(
            artObject,
            $"{DigitalRimGlowPrefix}Left",
            new Vector3(-0.78f * effectScale, 0.84f, -0.065f),
            new Vector3(0.025f, 1.08f * effectScale, 0.025f),
            accentColor
        );
        CreateOrUpdateDigitalEffectBar(
            artObject,
            $"{DigitalRimGlowPrefix}Right",
            new Vector3(0.78f * effectScale, 0.84f, -0.065f),
            new Vector3(0.025f, 1.08f * effectScale, 0.025f),
            accentColor
        );
        ...
        for (int i = 0; i < 4; i++)
        {
            float y = 0.36f + (i * 0.26f);
            float xJitter = i % 2 == 0 ? -0.04f : 0.05f;
            CreateOrUpdateDigitalEffectBar(
                artObject,
                $"{DigitalScanlinePrefix}{i}",
                new Vector3(xJitter, y, -0.055f),
                new Vector3(1.12f * effectScale, 0.012f, 0.012f),
                accentColor
            );
        }
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            float y = 0.42f + (i * 0.28f);
            CreateOrUpdateDigitalEffectBar(
                artObject,
                $"{DigitalGlitchBlockPrefix}{i}",
                new Vector3(side * (0.48f + i * 0.08f), y, -0.07f),
                new Vector3(0.16f + i * 0.04f, 0.055f, 0.018f),
                accentColor
            );
        }
    }

    void SetDogDigitalEffectChildrenVisible(GameObject artObject, bool visible)
    {
        if (artObject == null)
        {
            return;
        ...
        SetNamedChildVisible(artObject.transform, DigitalRimGlowName, visible);
        SetNamedChildVisible(artObject.transform, $"{DigitalRimGlowPrefix}Top", visible);
        SetNamedChildVisible(artObject.transform, $"{DigitalRimGlowPrefix}Bottom", visible);
        SetNamedChildVisible(artObject.transform, $"{DigitalRimGlowPrefix}Left", visible);
        SetNamedChildVisible(artObject.transform, $"{DigitalRimGlowPrefix}Right", visible);

        for (int i = 0; i < 4; i++)
        {
            SetNamedChildVisible(artObject.transform, $"{DigitalScanlinePrefix}{i}", visible);
        }

        for (int i = 0; i < 3; i++)
        {
            SetNamedChildVisible(artObject.transform, $"{DigitalGlitchBlockPrefix}{i}", visible);
        }
    }

    void SetNamedChildVisible(Transform parentTransform, string childName, bool visible)
    {
        ...
        {
            childTransform.gameObject.SetActive(visible);
        }
    }

    void CreateOrUpdateDigitalEffectBar(GameObject parentObject, string childName, Vector3 localPosition, Vector3 localScale, Color color)
    {
        ...
        Transform existingTransform = parentObject.transform.Find(childName);
        GameObject effectObject;

        if (existingTransform != null)
        {
            effectObject = existingTransform.gameObject;
        }
        else
        {
            effectObject = GameObject.CreatePrimitive(PrimitiveType.Cube);
            effectObject.name = childName;
            effectObject.transform.SetParent(parentObject.transform);
            PrepareRuntimePrimitive(effectObject);
        }
        ...
        Renderer effectRenderer = effectObject.GetComponent<Renderer>();
    ...
    {
        TintExistingDogIdentityArt(artObject, new Color(0.98f, 0.98f, 0.94f, 1f));
        TintExistingBreedArchetypeArt(artObject, new Color(0.88f, 0.92f, 0.92f, 0.95f));
        ApplyDogVisualModeEffects(artObject, true, DogVisualMode.SafeReal);
    }
    ...
    void TintDigitalImprintVisual(GameObject artObject, bool isDogA, float intensity)
    {
        ...
        Color copyColor = isDogA
            ...
            : new Color(0.34f, 0.72f, 1f, Mathf.Clamp01(0.45f + intensity * 0.45f));

        TintExistingDogIdentityArt(artObject, copyColor);
        TintExistingBreedArchetypeArt(artObject, copyColor);
        SetRuntimeObjectColor(artObject, copyColor);
        ApplyDogVisualModeEffects(artObject, isDogA, DogVisualMode.DigitalImprint);
    }

    void TintExistingDogIdentityArt(GameObject artObject, Color color)
    {
        SpriteRenderer spriteRenderer = GetExistingDogIdentitySpriteRenderer(artObject);
    ...
    {
        FaceDogIdentityArtsTowardPresentationCamera();
    ...
    void FaceDogIdentityArtsTowardPresentationCamera()
    {
        FaceBreedArchetypeArtTowardPresentationCamera(fighterADogIdentityArt);
        FaceBreedArchetypeArtTowardPresentationCamera(fighterBDogIdentityArt);
    }
    ...
    {
        // FighterA is on the left, so it faces right. FighterB is on the right, so it faces left.
        SetFighterFacing(fighterATransform, true);
        ...
        SetDogArtFacing(fighterADogIdentityArt, true);
        SetDogArtFacing(fighterBDogIdentityArt, false);
    ...
    void SetFighterFacing(Transform fighterTransform, bool faceRight)
    {
        if (fighterTransform == null)
        {
            return;
        ...
        fighterTransform.localRotation = Quaternion.Euler(0f, faceRight ? 90f : -90f, 0f);
    }
    ...
    {
        return artObject == fighterADogImprintArt ||
               artObject == fighterADogIdentityArt ||
               artObject == fighterABreedArchetypeArt;
    }
    ...
    void ApplyDogIdentityPortraitResultVisual(GameObject artObject, bool isWinner, bool isDraw, Color accentColor)
    {
        if (artObject == null || !artObject.activeSelf || !HasDogIdentityVisual(artObject))
        {
            return;
        }
        ...
        if (isWinner)
        {
            artObject.transform.localPosition += new Vector3(0f, 0.22f, -0.05f);
            artObject.transform.localScale *= 1.12f;
            TintExistingDogIdentityArt(artObject, Color.Lerp(Color.white, accentColor, 0.22f));
            SetDogArtFacing(artObject, faceRight);
            return;
        }
        ...
        if (isDraw)
        {
            artObject.transform.localPosition += new Vector3(0f, 0.08f, 0f);
            artObject.transform.localScale *= 1.04f;
            TintExistingDogIdentityArt(artObject, new Color(1f, 0.9f, 0.45f, 0.88f));
            SetDogArtFacing(artObject, faceRight);
            return;
        }

        artObject.transform.localPosition += new Vector3(0f, -0.18f, 0.06f);
        artObject.transform.localScale *= 0.84f;
        TintExistingDogIdentityArt(artObject, new Color(0.5f, 0.28f, 0.65f, 0.74f));
        SetDogArtFacing(artObject, faceRight);
    }
    ...
    void PositionScanSubjects()
    {
        if (scanDogATransform != null)
        {
            ...
            SetFighterFacing(scanDogATransform, true);
        }
        ...
        if (scanDogBTransform != null)
        {
            ...
            SetFighterFacing(scanDogBTransform, false);
        }

        UpdateScanDogVisualPositions();
    }
    ...
    GameObject GetOrCreateScanDogVisualArt(string objectName)
    {
        ...
        {
            artObject = existingObject.gameObject;
        }
        else
        {
            artObject = new GameObject(objectName);
            artObject.transform.SetParent(scanChamberRoot.transform);
        }
    ...
    void UpdateScanDogVisuals(Dog dogA, Dog dogB)
    {
        bool dogAHasArt = ApplyBreedArchetypeArtToScanDog(scanDogAVisualArt, dogA, true);
        bool dogBHasArt = ApplyBreedArchetypeArtToScanDog(scanDogBVisualArt, dogB, false);

        SetSafeDogCapsuleVisible(scanDogATransform, !dogAHasArt);
        SetSafeDogCapsuleVisible(scanDogBTransform, !dogBHasArt);
        UpdateScanDogVisualPositions();
    }
    ...
    bool ApplyBreedArchetypeArtToScanDog(GameObject artObject, Dog dog, bool isDogA)
    {
        ...
        if (!applied)
        {
            applied = ApplyBreedArchetypeArtToScanVisual(artObject, dog, isDogA, DogVisualMode.SafeReal);
        }
    ...
    bool CreateDigitalImprintVisual(GameObject artObject, Dog dog, bool isDogA)
    {
        ...
        if (!applied)
        {
            applied = ApplyBreedArchetypeArtToScanVisual(artObject, dog, isDogA, DogVisualMode.DigitalImprint);
        }
    ...
    bool ApplyBreedArchetypeArtToScanVisual(GameObject artObject, Dog dog, bool isDogA, DogVisualMode visualMode)
    {
        if (artObject == null || dog == null)
        {
            SetBreedArchetypeArtVisible(artObject, false);
        ...
        foreach (string resourceName in resourceNames)
        {
            ...
            if (sprite != null)
            {
                ConfigureBreedArchetypeSpriteArt(artObject, sprite, dog, isDogA, 1, 1, resourceName, visualMode);
            }
            else
            {
                ConfigureBreedArchetypeTextureArt(artObject, texture, dog, isDogA, 1, 1, resourceName, visualMode);
            }
    ...
    {
        UpdateSingleScanDogVisualPosition(scanDogAVisualArt, scanDogATransform, true);
        UpdateSingleScanDogVisualPosition(scanDogBVisualArt, scanDogBTransform, false);
    }

    void UpdateSingleScanDogVisualPosition(GameObject artObject, Transform safeDogTransform, bool isDogA)
    {
        if (artObject == null || safeDogTransform == null || !artObject.activeSelf)
        {
            return;
        }

        Vector3 safeDogPosition = safeDogTransform.localPosition;
        artObject.transform.localPosition = new Vector3(safeDogPosition.x, DogArtGroundY, safeDogPosition.z - 0.45f);
        artObject.transform.localRotation = Quaternion.identity;
    ...
    {
        if (safeDogTransform == null)
        {
            return;
        }

        safeDogTransform.gameObject.SetActive(visible);
    }
    ...
    void PositionScanDigitalCopies()
    {
        if (scanDigitalCopyA != null)
        {
            SetScanDigitalCopyTransform(scanDigitalCopyA, GetScanDigitalCopyStartPosition(true), 1f, true);
        }

        if (scanDigitalCopyB != null)
        {
            SetScanDigitalCopyTransform(scanDigitalCopyB, GetScanDigitalCopyStartPosition(false), 1f, false);
        }
    }

    Vector3 GetScanDigitalCopyStartPosition(bool isDogA)
    {
    ...
    void SetScanDigitalCopyTransform(GameObject copyObject, Vector3 position, float pulse, bool isDogA)
    {
        ...
        copyObject.transform.localScale = HasDogVisualArt(copyObject)
    ...
    bool HasDogIdentityVisual(GameObject artObject)
    {
        SpriteRenderer spriteRenderer = GetExistingDogIdentitySpriteRenderer(artObject);
        return spriteRenderer != null && spriteRenderer.enabled && spriteRenderer.sprite != null;
    }

    bool HasDogVisualArt(GameObject artObject)
    {
        return HasDogIdentityVisual(artObject) || HasBreedArchetypeVisual(artObject);
    }
    ...
    void ApplyDogIdentityVisuals(GameObject artObject, Dog dog, int currentHealth, int maxLikelyHealth, bool isFighterA)
    {
        ...
        SetDogArtFacing(artObject, isFighterA);
        GroundAlignDogArt(artObject, DogArtGroundY);
        ApplyDogVisualModeEffects(artObject, isFighterA, DogVisualMode.ArenaFighter);
    }
    ...
    GameObject CreateFighterPlaceholder(string objectName, Vector3 position, Color color)
    {
        ...
        SetFighterFacing(fighter.transform, position.x < 0f);
    ...
    GameObject CreateSafeDogPlaceholder(string objectName, Vector3 position, Color color)
    {
        ...
        SetFighterFacing(safeDog.transform, position.x < 0f);
    ...
    void CreateTraitEffectsIfNeeded()
    {
        if (arenaRoot == null)
        {
            return;
    ...
    void HideTraitEffects()
    {
        SetImpactEffectActive(traitAuraA, false);
        SetImpactEffectActive(traitAuraB, false);
        SetImpactEffectActive(traitSlashA, false);
        SetImpactEffectActive(traitSlashB, false);
    }
    ...
    )
    {
        ...
        GameObject auraObject = isFighterA ? traitAuraA : traitAuraB;
        GameObject slashObject = isFighterA ? traitSlashA : traitSlashB;
        float healthPercent = GetVisualHealthPercent(currentHealth, maxLikelyHealth);
        float pulse = Mathf.PingPong(Time.time * 3.2f, 1f);

        bool hasAura = false;
        Color auraColor = Color.white;
        float auraScale = 1f;

        if (dog.HasTrait(DogTrait.Durable) && incomingImpact > 0)
        {
            BlendTraitAura(ref hasAura, ref auraColor, ref auraScale, new Color(0.28f, 1f, 0.84f), 1.08f);
        }

        if (dog.HasTrait(DogTrait.Clutch) && (healthPercent <= 0.35f || roundNumber >= 5))
        {
            float clutchStrength = healthPercent <= 0.35f ? 1.28f : 1.12f;
            BlendTraitAura(ref hasAura, ref auraColor, ref auraScale, Color.Lerp(new Color(1f, 0.72f, 0.12f), Color.white, pulse * 0.35f), clutchStrength);
        }

        if (dog.HasTrait(DogTrait.LateBloomer) && roundNumber >= 3)
        {
            float growth = Mathf.InverseLerp(3f, 6f, roundNumber);
            BlendTraitAura(ref hasAura, ref auraColor, ref auraScale, new Color(0.3f, 0.95f, 0.7f), Mathf.Lerp(1.05f, 1.28f, growth));
        }

        if (dog.HasTrait(DogTrait.Prodigy) && ShouldShowProdigyGlow(roundNumber, outgoingImpact, incomingImpact))
        {
            BlendTraitAura(ref hasAura, ref auraColor, ref auraScale, new Color(0.86f, 1f, 1f), 1.18f);
        }

        if (hasAura)
        {
            ShowTraitAuraEffect(auraObject, fighterTransform, auraColor, auraScale, isFighterA);
        }

        if (dog.HasTrait(DogTrait.Aggressive) && outgoingImpact > 0)
        {
            float intensity = Mathf.InverseLerp(1f, 30f, outgoingImpact);
            ShowTraitSlashEffect(
                slashObject,
                fighterTransform,
                Color.Lerp(new Color(1f, 0.22f, 0.16f), new Color(1f, 0.28f, 0.78f), intensity * 0.35f),
                Mathf.Lerp(0.9f, 1.35f, intensity),
                isFighterA,
                25f
            );
            return;
        ...
        if (dog.HasTrait(DogTrait.GlassCannon) && ShouldShowGlassCannonFlicker(roundNumber, outgoingImpact, incomingImpact))
        {
            float flicker = Mathf.PingPong((Time.time * 5.7f) + roundNumber, 1f);
            ShowTraitSlashEffect(
                slashObject,
                fighterTransform,
                Color.Lerp(new Color(0.95f, 1f, 1f), new Color(0.95f, 0.12f, 1f), flicker),
                Mathf.Lerp(0.78f, 1.12f, flicker),
                isFighterA,
                -18f
            );
        }
    ...
    void BlendTraitAura(ref bool hasAura, ref Color auraColor, ref float auraScale, Color addedColor, float addedScale)
    {
        if (!hasAura)
        {
            hasAura = true;
            auraColor = addedColor;
            auraScale = addedScale;
            return;
        }

        auraColor = Color.Lerp(auraColor, addedColor, 0.5f);
        auraScale = Mathf.Max(auraScale, addedScale);
    }

    bool ShouldShowProdigyGlow(int roundNumber, int outgoingImpact, int incomingImpact)
    {
        return roundNumber == 1 || ((roundNumber % 3) == 0 && (outgoingImpact > 0 || incomingImpact > 0));
    }

    bool ShouldShowGlassCannonFlicker(int roundNumber, int outgoingImpact, int incomingImpact)
    {
        return outgoingImpact >= 8 || incomingImpact >= 8 || (roundNumber % 2) == 1;
    }

    void ShowTraitAuraEffect(GameObject auraObject, Transform fighterTransform, Color color, float scale, bool isFighterA)
    {
        if (auraObject == null || fighterTransform == null)
        {
            return;
        }
    ...
    {
        if (slashObject == null || fighterTransform == null)
        {
            return;
        }
    ...
    {
        bool hasGenericImprintArt = dogImprintPrefab != null && fighterADogImprintArt != null && fighterBDogImprintArt != null;
        bool hasBreedArt = HasDogVisualArt(fighterABreedArchetypeArt) || HasDogVisualArt(fighterBBreedArchetypeArt);
        return hasGenericImprintArt || hasBreedArt;
    }
